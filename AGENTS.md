...
The classifier has been split into `paypal_fee_crawler/classify/`. Most data
constants live in `crawler/src/paypal_fee_crawler/classify/patterns.py`:

- `_TABLE_CATEGORY_KEYWORDS` for product-specific rate-table keywords.
- `_PRODUCT_ALIASES` for product-name matching in row cells.
- `_FIXED_FEE_KEYWORDS`, `_DIRECT_FIXED_FEE_KEYWORDS`, and
  `_INTERNATIONAL_SURCHARGE_KEYWORDS` for identifying fixed-fee and
  international surcharge tables.
...
- APM method token sets live in `_APM_METHOD_MATCHERS`, with region tokens for
  bank-transfer variants and generic header phrases/tokens in
  `_APM_HEADER_PHRASES` / `_APM_HEADER_TOKENS` to suppress noise from
  variant-only rows.

Logic modules (`products.py`, `variants.py`, `conditions.py`, `schedules.py`,
`references.py`, `rules.py`, `apm.py`) import these constants from `patterns.py`.
For example:

- `_resolve_reference` in `classify/references.py` resolves textual schedule
  references by matching the referenced product family and, when there are
  multiple candidates, by the source rule's `variant_id` and `conditions`. The
  source market is only injected when a candidate explicitly lists that market,
  so generic target rules still match when no market-specific rule exists.
- `_extract_apm_methods` in `classify/apm.py` performs token-based APM method
  extraction.
...
- 4,006 transaction fee rules
- 176 currency-conversion entries
- 4,182 total core entries
- 1,616 inherited schedule objects
- 1,925 inherited schedule references
